...
// Variáveis ​​globais para armazenar a classificação do processo e o tamanho
//do comunicador
int rank, size;

// Número de pontos de um lado. O número total de pontos
// será p_count * p_count.
constexpr int p_count = 512;

// Outras variáveis ​​globais. Nós os lemos na linha de comando
corte interno;
...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        for (int i = 0; i < total_points; ++i) {
          printf("%d%s", mset[i], (i % p_count == p_count - 1) ? "\n" : " ");
        }
        delete[] points;
        delete[] mset;
    }
